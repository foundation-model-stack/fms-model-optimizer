...
def parse_args():
    ...
    # --- [fms_mo] added to perform different tasks
    parser.add_argument(
        "--do_qat",
        action="store_true",
        help="perform QAT func",
    )
    parser.add_argument(
        "--do_ptq",
        action="store_true",
        help="perform PTQ func",
    )
    parser.add_argument(
        "--pact_a_lr",
        type=float,
        default=1e-4,
        help="LR for activation quantizer",
    )
    parser.add_argument(
        "--attn_impl",
        type=str,
        default="eager",
        help="['eager', 'sdpa', 'flashattention2']",
    )
    parser.add_argument(
        "--do_lowering",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Num of batches to be fetched from dataloader.",
    )
    parser.add_argument(
        "--ptq_batchsize",
        type=int,
        default=12,
        help="Batch size to be used. Usually same as dataloader.batchsize but could be different.",
    )
    parser.add_argument(
        "--ptq_nouterloop", type=int, default=1000, help="Num of outer loop for PTQ"
    )
    parser.add_argument(
        "--ptq_ninnerloop",
        type=int,
        default=1,
        help="Total number of inner loops to perform.",
    )
    parser.add_argument(
        "--ptq_coslr", type=str, default="", help="which optimizer to use cos lr"
    )
    parser.add_argument(
        "--ptq_lrw", type=float, default=1.0e-5, help="learning weight w"
    )
    parser.add_argument(
        "--ptq_lrcv_w",
        type=float,
        default=1.0e-3,
        help="learning rate clip value weight",
    )
    parser.add_argument(
        "--ptq_lrcv_a",
        type=float,
        default=1.0e-3,
        help="learning rate clip value activation",
    )
    parser.add_argument(
        "--ptqmod_to_be_optimized",
        nargs="*",
        default=[],
        help="list of modules to be optimized",
    )
    parser.add_argument(
        "--ptq_loss_func", type=str, default="mse", help="loss function for PTQ"
    )
...
def main():
    ...
    accelerator = Accelerator(
        ...
        **accelerator_log_kwargs,
        mixed_precision="fp16",  # if args.do_qat else 'no' # [fms_mo] force fp16 for faster demo
    )
    ...
    logging.basicConfig(
        ...
        datefmt="%m/%d/%Y %H:%M:%S",
        level=logging.INFO,
        filename=f"./fms_mo.qat_demo.log",  # [fms_mo] add 'filename' to dump log to file
    )
    ...
    if args.with_tracking:
        ...
        accelerator.init_trackers(
            "QAT_logs" if args.do_qat else "qa_no_trainer",  # experiment_config
        )
    ...
    # Evaluation --- This block of codes was placed after training for-loop (original file ~Line 950)
    #                   and only run once without a "func def"
    #                   we make it a func and move up here, so that we can call during training easily
    def squad_eval(model, keep_model_in_eval_mode=True):
        ...
        model.eval()
        mod_dev = model.device
        # Third Party
        from tqdm import tqdm

        for batch in tqdm(eval_dataloader):
            with torch.no_grad():
                outputs = model(**batch.to(mod_dev))
        ...
        # logger.info(f"mean logits: {start_logits_concat.mean()} {end_logits_concat.mean()}")
        ...
        if not keep_model_in_eval_mode:
            # NOTE original code keeps model in eval mode,
            #       add this flag for in-training evals.
            model.train()
    ...
    if args.do_qat:
        ...
        exam_inp = [
            next(iter(train_dataloader)) for _ in range(qcfg["qmodel_calibration"])
        ]
    ...
    if args.do_ptq:
        ...
        logger.info(f"--- Accuracy of {args.model_name_or_path} after PTQ")
        squad_eval(model)

        # assume single GPU, otherwies need to unwrap, see example near the end of the training loop
        ...
        def speedtest(model, exam_inp, Ntest=100):
            with torch.no_grad():
                ...
                for _ in range(Ntest):
                    ...
                    model(**exam_inp)
                    runtime = time.perf_counter() - st_time
                    runtimes_ms.append(runtime * 1000)

            return np.median(runtimes_ms)

        exam_inp = next(
            iter(train_dataloader)
        )  # only need 1 batch for torch.compile use
        summary = {
            "precision": [],
            "compile mode": [],
            "runtime/inference (sec)": [],
            "f1": [],
        }
        del exam_inp["start_positions"], exam_inp["end_positions"]
        for label, comp_mode in [
            ("fp16", False),
            ("fp16", "ind"),
            ("fp16", "cugr"),
            ("int8", False),
            ...
            if label == "int8":
                ...
                qmodel_prep(
                    ...
                    optimizer,
                    use_dynamo=True,
                    ckpt_reload=args.model_name_or_path,
                )

                mod2swap = [
                    n for n, m in model_copy.named_modules() if isinstance(m, QLinear)
                ]
            ...
            if comp_mode is not False:
                ...
                with torch.no_grad():
                    model_copy = torch.compile(
                        model_copy,
                        mode="reduce-overhead" if comp_mode == "cugr" else "default",
                    )
                    model_copy(**exam_inp)
            ...
            summary["runtime/inference (sec)"].append(med_runtime)
            summary["f1"].append(metrics["f1"])

            del model_copy

        logger.info(
            f"\n--- Speed test based on ckpt {args.model_name_or_path} "
            f"using batch size {args.per_device_train_batch_size}"
        )
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            # --- [fms_mo] add TB logging for debugging ---
            if args.with_tracking and (
                completed_steps % 100 == 0
                or completed_steps == args.max_train_steps - 1
            ):
                dict2log = {
                    k: v.item() for k, v in model.named_parameters() if "clip_val" in k
                }
                dict2log["training_loss"] = loss.item()
                for i, pg_i in enumerate(optimizer.param_groups):
                    dict2log[f"lr_{i}"] = pg_i["lr"]
                if completed_steps % 1000 == 0 or (
                    args.max_train_steps - completed_steps < 1000
                    and completed_steps % 100 == 0
                ):
                    eval_metric = squad_eval(model, keep_model_in_eval_mode=False)
                    # normally model will remain in eval mode after calling eval unless we say otherwise
                    dict2log["f1"] = eval_metric["f1"]

                accelerator.log(dict2log, step=completed_steps)
    ...
    if args.output_dir is not None:
        ...
        unwrapped_model.save_pretrained(
            ...
            is_main_process=accelerator.is_main_process,
            save_function=accelerator.save,
            safe_serialization=False,  # or it will complain about qkv clipvals being "sync'ed"
        )
        if accelerator.is_main_process:
            tokenizer.save_pretrained(args.output_dir)
            if args.push_to_hub:
